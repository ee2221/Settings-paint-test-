...
import { X, Lock, Eye, EyeOff, GripHorizontal } from 'lucide-react';
...
const ObjectProperties: React.FC = () => {
  ...
  // Initialize position to avoid overlapping with other panels
  useEffect(() => {
    if (panelRef.current && position.x === 0 && position.y === 0) {
      const rect = panelRef.current.getBoundingClientRect();
      // Position it in the center-right area, avoiding the top-right corner
      const centerX = window.innerWidth - rect.width - 16;
      const centerY = Math.max(120, (window.innerHeight - rect.height) / 2); // Start below top panels
      setPosition({ x: centerX, y: centerY });
    }
  }, [selectedObject]); // Reposition when object is selected
  ...
  return (
    <div 
      ref={panelRef}
      className="fixed bg-[#1a1a1a] rounded-xl shadow-2xl shadow-black/20 border border-white/5 max-h-[85vh] overflow-hidden z-40"
      style={{
        left: `${position.x}px`,
        top: `${position.y}px`,
        width: '256px'
