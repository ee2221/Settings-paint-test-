...
const SettingsPanel: React.FC = () => {
  ...
  // Initialize position to top left, accounting for other UI elements
  useEffect(() => {
    if (panelRef.current && position.x === 0 && position.y === 0) {
      // Position at top left with enough margin to avoid other UI elements
      // Account for the hide interface button and save button that are also at top left
      setPosition({ x: 200, y: 16 }); // 200px from left to avoid other buttons, 16px from top
    }
  }, [isOpen]);
  ...
  // Settings button (always visible) - positioned at top left
  if (!isOpen) {
    return (
      <button
        onClick={() => setIsOpen(true)}
        className="fixed top-4 left-52 p-3 bg-[#1a1a1a] hover:bg-[#2a2a2a] rounded-xl shadow-2xl shadow-black/20 border border-white/5 transition-all duration-200 hover:scale-105 z-50"
        title="Open Settings"
      >
